...
export class ArchiveScene {
  ...
  async load() {
    ...
    const gltf = await new GLTFLoader().loadAsync(
      assetUrl("assets/archive-cassette.glb"),
    );
  ...
  async createAssemblyModel() {
    this.assemblyTemplate ??= new GLTFLoader()
      .loadAsync(assetUrl("assets/archive-assembly.glb"))
      .then((gltf) => {
        gltf.scene.updateMatrixWorld(true);
        return gltf.scene;
      })
    ...
    const shift = {
      lane:
        Math.abs(this.selectedCell.lane) > 2048
          ...
          : 0,
      // Unequal lane lengths have no shared eight-row period.
      row: 0,
    };
  ...
  private drawLabel(index: number) {
    ...
    c.fillText("E_POWER", 22, 116);
    c.font = "32px MiSans";
    c.fillStyle = "#878476";
    c.fillText("KNOWLEDGE ARCHIVE", 25, 174);
    c.fillStyle = "#171713";
